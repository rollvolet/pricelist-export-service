...
function attachTaskResultsQuery (task, results, graph = MU_APPLICATION_GRAPH) {
  ...
  const containerPartsStatements = results.map((result) => {
    return `${sparqlEscapeUri(result)} nfo:belongsToContainer ${sparqlEscapeUri(containerUri)} .`
  }).join('\n');
...
SELECT (?task as ?uri) (?uuid as ?id) ?created ?used ?operation WHERE {
    GRAPH ${sparqlEscapeUri(graph)} {
        ?task a task:Task ;
            ...
            mu:uuid ?uuid .
        OPTIONAL { ?task task:inputContainer ?container. ?used nfo:belongsToContainer ?container . }
